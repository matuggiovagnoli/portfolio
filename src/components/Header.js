    ...
    <header className="fixed w-full flex justify-between items-center p-4 bg-transparent text-black z-50">
      ...
      <div className="flex space-x-2">
        <button 
          ...
          className={`w-9 h-9 lg:h-12 lg:w-12 md:w-10 md:h-10 text-sm md:text-base lg:text-lg  px-2 py-1 rounded ${i18n.language === 'en' ? 'bg-black text-white' : 'bg-transparent text-black border border-black'}`}
        >
          EN
        </button>
        <button 
          onClick={() => changeLanguage('es')} 
          className={`w-9 h-9 lg:h-12 lg:w-12 md:w-10 md:h-10 text-sm md:text-base lg:text-lg  px-2 py-1 rounded ${i18n.language === 'es' ? 'bg-black text-white' : 'bg-transparent text-black border border-black'}`}
        >
          ES
        </button>
      </div>
